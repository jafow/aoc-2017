...
appear, including { and }. Within garbage, < has no special
meaning.
...
def streams(line):
    """ push and pop a stack for each matching brace; sum the
    depth count by the stack length for each pair closed; this only
    works because we know these are well-formed..."""
    stack = []
    levels = 0
    for c in line:
        if c == '{':
...
INPUT = open('./input/9.txt').readline()
ignores = re.sub(r'!.', '', INPUT)
braces = re.sub(r'<.*?>', '', ignores)

# p1
streams(braces)

# pt2: add <> back to the braces regex replace
garb = re.sub(r'<.*?>', '<>', ignores)
print(len(ignores) - len(garb))

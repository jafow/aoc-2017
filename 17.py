...
values (including 0) in the circular
buffer.
...
position after each iteration of the
algorithm):
...
# plan:
# create a list L, looping around itself N times,
# adding a value I the last index until stop
# repeat
# keep track of 3 values:
# 1. the current index of 0, Z
# 2. the index of the currently added value
# 3. the value at Z + 1


def spin_lock(N, start=0, stop=2017):
    p = 0
    zero_index = 0
    neighbor = 0
    ...
    for i in range(1, stop + 1):
        ...
        if ((p - 1) % i) == zero_index:
            # the value to the right of 0 is i
            neighbor = i

        if ((p - 1) % i) == zero_index - 1:
            # value is inserted to the left of the index of Z
            # so we increment its index as it scoots to the right
            zero_index += 1
...
start = time.time()
s = spin_lock(329, 0, 50000000)
stop = time.time()

print('runtime {0} res {1}'.format(stop - start,  s))


class TestDay17(unittest.TestCase):
    def test_fills_list_after_circling(self):
        N = 3
        self.assertEqual(spin_lock(N, 0), 638)
